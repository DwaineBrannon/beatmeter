...
export const rateAlbum = async (albumData, ratingData) => {
  return executeFirestoreOperation(
    async () => {
      ...
      const existingRating = await getUserAlbumRating(
        ratingData.userId,
        albumId
      );
      if (existingRating) {
        throw new Error(
          'User has already rated this album. Use updateAlbumRating instead.'
        );
      }
      ...
      await updateAlbumRating(albumId, ratingData.rating);

      // 5. Create activity for the feed
      const ratingWithId = { ...ratingData, id: ratingId, albumId: albumId };
 ...
 */
export const updateExistingAlbumRating = async (
  ratingId,
  updateData,
  albumData
) => {
  ...
  return executeFirestoreOperation(
    async () => {
      ...
      const albumData = {
        ...
        duration:
          spotifyAlbumData.tracks?.items?.reduce(
