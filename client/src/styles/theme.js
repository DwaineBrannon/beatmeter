export const theme = {
  colors: {
    background: {
      primary: 'rgba(22, 25, 27, 0.95)',
      secondary: '#1a1a1a',
      accent: 'rgba(101, 15, 151, 0.16)'
    },
    surface: {
      primary: '#282828',
      secondary: 'rgba(50, 47, 53, 0.8)',
      tertiary: '#e9ecef'
    },
    text: {
      primary: '#fff',
      secondary: 'rgba(255,255,255,0.7)',
      disabled: '#6c757d'
    },
    primary: {
      main: '#007bff',
      dark: '#0056b3'
    },
    border: {
      primary: '#ddd',
      secondary: '#ccc'
    },
    accent: '#007bff',
    secondary: 'rgba(71, 103, 125, 0.31)',
    star: {
      filled: '#ffc107',  /* Gold star color */
      hover: '#e0a800'    /* Darker gold for hover */
    }
  },
  spacing: {
    small: '8px',
    medium: '16px',
    large: '24px',
    xlarge: '80px'
  },
  borderRadius: {
    small: '8px',
    medium: '12px',
    large: '16px'
  },
  shadows: {
    small: '0 2px 8px rgba(0, 0, 0, 0.2)',
    medium: '0 4px 12px rgba(0, 0, 0, 0.3)'
  },
  transitions: {
    fast: '150ms ease',
    normal: '250ms ease',
    slow: '350ms ease'
  },
  buttons: {
    primary: {
      background: 'rgba(83, 142, 181, 0.31)',
      text: '#fff',
      hoverBackground: 'rgba(40, 168, 61, 0.31)',
    },
    secondary: {
      background: 'rgba(0, 0, 0, 0.8)',
      text: '#fff',
      hoverBackground: 'rgba(0, 0, 0, 0.9)',
    }
  }
};